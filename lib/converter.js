...
Converter.prototype.fillResize = function (width, height, orientation) {
    this.operation.width = width;
    this.operation.height = height;
    this.operation.type = imgr.FILL;
    this.operation.orientation = orientation || this.options.orientation;
    return this;
};
...
Converter.prototype.save = function (output, callback) {
    ...
    mkdirp(dir, function () {
        ...
        self.size(self.image, function (err, size) {
            ...
            //Adaptive filling
            if (operation.type === imgr.FILL && operation.width && operation.height) {
                var original_ar = size.width / size.height
                  , original_width = operation.width
                  , original_height = operation.height
                  , new_ar = operation.width / operation.height
                  , fill_offset = 0
                  , fill_width = 0
                  , fill_height = 0;
                if (new_ar < original_ar) {
                    operation.height = Math.round(size.height / (size.width / operation.width));
                    fill_height = (original_height - operation.height) / 2;
                } else if (new_ar > original_ar) {
                    operation.width = Math.round(size.width / (size.height / operation.height));
                    fill_width = (original_width - operation.width) / 2;
                }

            //Adaptive resizing
            } else if (operation.width && operation.height && typeof operation.crop_width === 'undefined') {
                var original_ar = size.width / size.height
                  , new_ar = operation.width / operation.height
                  , crop_offset = 0;
                if (new_ar < original_ar) {
            ...
            if (!operation.width && operation.height) {
                operation.width = operation.height === size.height
                                ? size.width
                                : Math.ceil((operation.height / size.height) * size.width)
                                ;
            }
            if (!operation.height && operation.width) {
                operation.height = operation.width === size.width
                                 ? size.width
                                 : Math.ceil((operation.width / size.width) * size.height)
                                 ;
            }
            ...
            if (operation.type === imgr.FILL) {
                image.borderColor('#ffffff').border(fill_width,fill_height);
            }
...
utils.concurrent(Converter.prototype, 'size', 10);
utils.floodProtection(Converter.prototype, 'size');
utils.concurrent(Converter.prototype, 'save', 10);
